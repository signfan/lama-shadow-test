...
def load_image(fname, mode='RGB', return_orig=False):
    img = np.array(ImageOps.exif_transpose(Image.open(fname).convert(mode)))
    if img.ndim == 3:
...
class InpaintingDataset(Dataset):
    ...
    def __getitem__(self, i):
        ...
        if self.pad_out_to_modulo is not None and self.pad_out_to_modulo > 1:
            result['shape'] = np.array(image.shape[1:])
            result['image'] = pad_img_to_modulo(result['image'], self.pad_out_to_modulo)
            result['mask'] = pad_img_to_modulo(result['mask'], self.pad_out_to_modulo)

        #mask2 = result['mask'].copy()
        #kernel_size = 10
        #kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (kernel_size, kernel_size))
        #mask2[0] = cv2.dilate(mask2[0], kernel, iterations=1)

        #result2 = dict(image=result['image'], mask=mask2)

        return result#, result2

class InpaintingHSVDataset(Dataset):
    def __init__(self, datadir, img_suffix='.jpg', pad_out_to_modulo=None, scale_factor=None):
        self.datadir = datadir
        self.mask_filenames = sorted(list(glob.glob(os.path.join(self.datadir, '**', '*mask*.png'), recursive=True)))
        self.img_filenames = [fname.rsplit('_mask', 1)[0] + img_suffix for fname in self.mask_filenames]
    ...
    def __getitem__(self, i):
        image = load_image(self.img_filenames[i], mode='HSV')
        mask = load_image(self.mask_filenames[i], mode='L')
        result = dict(image=image, mask=mask[None, ...])
