...
class DefaultInpaintingTrainingModule(BaseInpaintingTrainingModule):
    def __init__(self, *args, concat_mask=True, rescale_scheduler_kwargs=None, image_to_discriminator='predicted_image',
        ...
        #self.shadow = (args[0]['data']['train']['kind'] == 'shadow')
        try:
            kind = args[0]['data']['train']['kind']
            self.shadow = kind in ['shadow', 'shadow_HSV', 'shadow_aug']
            print('1')
        except:
            self.shadow = False
            print('2')
    ...
    def forward(self, batch):
        if self.training and self.rescale_size_getter is not None:
            ...
            if 'target' in batch.keys():
                batch['target'] = F.interpolate(batch['target'], size=cur_size, mode='bilinear', align_corners=False)
    ...
    def discriminator_loss(self, batch):
        ...
        if batch.get('use_fake_fakes', False):
            ...
            self.adversarial_loss.pre_discriminator_step(real_batch=img, fake_batch=fake_fakes,
                                                         generator=self.generator, discriminator=self.discriminator)
