...
from saicinpainting.evaluation.data import InpaintingDataset as InpaintingEvaluationDataset, \
    OurInpaintingDataset as OurInpaintingEvaluationDataset, ceil_modulo, InpaintingEvalOnlineDataset, \
        InpaintingHSVDataset as InpaintingEvaluationHSVDataset
...
class InpaintingTrainDataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        return dict(image=img,
                    mask=mask)
#shadow
class InpaintingShadowTrainDataset(Dataset):
    def __init__(self, indir, transform):
        self.in_files = list(glob.glob(os.path.join(indir, 'train_A', '*.png'), recursive=True))
        self.in_masks = list(glob.glob(os.path.join(indir, 'train_B', '*.png'), recursive=True))
        self.in_target = list(glob.glob(os.path.join(indir, 'train_C_fixed_official', '*.png'), recursive=True))
        #self.mask_generator = mask_generator
        self.transform = transform
        self.iter_i = 0
    ...
    def __getitem__(self, item):
        ...
        # TODO: maybe generate mask before augmentations? slower, but better for segmentation-based masks
        #mask = self.mask_generator(img, iter_i=self.iter_i)
        mpath = self.in_masks[item]
        mask = cv2.imread(mpath)
        mask = np.transpose(mask, (2, 0, 1))
        mask = np.expand_dims(mask[0], 0)
        mask = (mask/255).astype(np.float32)
        tpath = self.in_target[item]
        target = cv2.imread(tpath)
...
class InpaintingShadowTrainHSVDataset(Dataset):
    def __init__(self, indir, transform):
        self.in_files = list(glob.glob(os.path.join(indir, 'train_A', '*.png'), recursive=True))
        self.in_masks = list(glob.glob(os.path.join(indir, 'train_B', '*.png'), recursive=True))
        self.in_target = list(glob.glob(os.path.join(indir, 'train_C_fixed_official', '*.png'), recursive=True))
    ...
    def __getitem__(self, item):
        ...
        target = self.transform(image=target)['image']
        target = np.transpose(target, (2, 0, 1))
        target[0] = (target[0] * 255) / 180
        self.iter_i += 1
        return dict(image=img,
                    mask=mask, target=target)

class InpaintingAugmentedShadowTrainDataset(Dataset):
    def __init__(self, indir, transform, darken_kwargs):
        #self.in_files = list(glob.glob(os.path.join(indir, 'train_A', '*.png'), recursive=True))
        self.in_masks = list(glob.glob(os.path.join(indir, 'train_B', '*.png'), recursive=True))
        self.in_target = list(glob.glob(os.path.join(indir, 'train_C_fixed_official', '*.png'), recursive=True))
        #self.mask_generator = mask_generator
        self.transform = transform
        self.opt = darken_kwargs
        self.iter_i = 0
    ...
    def __getitem__(self, item):
        ...
        target = self.transform(image=target)['image']
        target = np.transpose(target, (2, 0, 1))
        dark_target = darken(target, self.opt)
        img = mask * dark_target + (1 - mask) * target
        # print('target: ', target, target.shape)
        # print('image: ', img, img.shape)
        # print('equal:not', np.sum(target==img), np.sum(target!=img))
        self.iter_i += 1
        return dict(image=img,
                    mask=mask, target=target)
...
def make_default_train_dataloader(indir, kind='default', out_size=512, mask_gen_kwargs=None, transform_variant='default',
                                  mask_generator_kind="mixed", dataloader_kwargs=None, ddp_kwargs=None, darken_kwargs=None, **kwargs):
    ...
    if kind == 'default':
        ...
        dataset = InpaintingShadowTrainDataset(indir=indir, transform=transform, **kwargs)
    elif kind == 'shadow_HSV':
        dataset = InpaintingShadowTrainHSVDataset(indir=indir, transform=transform, **kwargs)
    elif kind == 'shadow_aug':
        dataset = InpaintingAugmentedShadowTrainDataset(indir=indir, transform=transform, darken_kwargs=darken_kwargs, **kwargs)
    else:
...
def make_default_val_dataset(indir, kind='default', out_size=512, transform_variant='default', **kwargs):
    ...
    if kind == 'default':
        ...
        dataset = InpaintingEvaluationHSVDataset(indir, **kwargs)
    elif kind == 'our_eval':
